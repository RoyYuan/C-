...
int length(stu *head);    //获取链表长度
int insert_end(stu *head, stu *n); //链表尾插
int del_node(stu *head);  //删除结点函数
int insert_cur(stu *head, char id[30], stu *n);  //在链表某个位置进行插入
...
int comp_name(const void *x, const void *y);//按姓名进行排序
int comp_math(const void *x, const void *y);//按数学成绩进行排序
int comp_PE(const void *x, const void *y);//按体育成绩进行排序
int comp_english(const void *x, const void *y);//按英语成绩进行排序
int comp_computer(const void *x, const void *y);//按计算机成绩进行排序
int comp_C_program(const void *x, const void *y);//按C语言成绩进行排序
int comp_score(const void *x, const void *y);//按总分进行排序
/*每一个功能一个控制函数, 返回管理员界面*/
...
gpointer create_entry(gint max, gboolean editable, gboolean visible);//创建文本框函数（最大长度、可编译性、可视性）
...
void login_admin_change_message_view(GtkWidget *widget, gpointer data);//对不同更改结果的表达
...
int insert_end(stu *head, stu *p){        //尾插（将新结点插在head结点与head之前的结点之间
    if(p == NULL || head == NULL){
        return 1;
    }
    p -> prev = head -> prev;
...
stu *find_C_program_exist(stu *head, char id[30]){
    ...
    p = head -> next;
    while(p != head){
        if(!strcmp(id, p -> score.C_program)){
            return p;
        }
        p = p -> next;
    }
...
int del_id(stu *head, char id[30]){                    //删除结点
    stu *temp = NULL;
    temp = find_id_exist(head, id);                    //找到指定id的文件地址
    if(!head || !temp)                                 //如果头结点为NULL或没找到对应的id（temp为NULL）
        return 1;
...
int stu_exist_mod(char id[30], char password[30]){    //判断学生id是否存在
    stu *temp = NULL;
	temp = head -> next;
    while(temp != head){
...
void login_identify_code_message_view(int count){         //对验证码不同结果的表达
    ...
    message = gtk_message_dialog_new(NULL,GTK_DIALOG_MODAL,GTK_MESSAGE_INFO,GTK_BUTTONS_NONE,NULL); //特定信息框显示一个问号
...
void login_ctrl(GtkWidget *widget,gpointer data)        //登陆control
{
	char id[30]="\0";									//定义id用来判断
	char password[30]="\0";								//定义password用于判断

	gtk_widget_hide(login.window);                      //隐藏登录窗口
	if(!strcmp("管理员",nameid)){			            //判断登录身份是否时管理员
		strcpy(id,gtk_entry_get_text(GTK_ENTRY(login.id)));   //将文本框中的值赋给变量
		strcpy(password,gtk_entry_get_text(GTK_ENTRY(login.password)));
		if(!strcmp(id,manager->id)&&!strcmp(password,manager->password)){
			if(!strcmp(gtk_label_get_text(GTK_LABEL(login.label)),gtk_entry_get_text(GTK_ENTRY(login.identify_code)))){ //对验证码的匹配处理
				count=0;
		...
		strcpy(password,gtk_entry_get_text(GTK_ENTRY(login.password)));
		if(stu_exist_mod(id,password)){
				if(!strcmp(gtk_label_get_text(GTK_LABEL(login.label)),gtk_entry_get_text(GTK_ENTRY(login.identify_code)))){
					count=0;
					login_student_view();
				}
				else{
...
void login_change_ret_ctrl(GtkWidget *widget, gpointer data){      //从更改学生信息窗口返回管理员窗口
    gtk_widget_hide(change.window);
    gtk_widget_show(login_admin.window);
}
...
void login_exit_ctrl(GtkWidget *widget , gpointer data){            //退出管理员菜单的实现
	GtkWidget *message;
	GtkWidget *button;
    if(!strcmp("管理员", nameid)){                              //判断是否是管理员，显示不同的界面
        login_admin_view();
    }
    else{
        login_student_view();
    ...
    if(flag == 0){
        message = gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL,GTK_MESSAGE_INFO, GTK_BUTTONS_NONE, NULL);//message特定显示符号
        gtk_message_dialog_set_markup(GTK_MESSAGE_DIALOG(message), "你确定要退出么？");
        button = gtk_button_new_from_stock(GTK_STOCK_OK);                                 //特定的确定键
        if(!strcmp("管理员", nameid)){
            g_signal_connect(button, "clicked", G_CALLBACK(login_admin_ret_ctrl), NULL);  //如果点击退出管理员菜单则回调返回登陆界面的函数
		}
        else{
            g_signal_connect(button, "clicked", G_CALLBACK(login_stu_ret_ctrl),NULL);     //退出学生菜单
        }
        g_signal_connect_swapped(button, "clicked", G_CALLBACK(gtk_widget_destroy), message); //如果点X的话关掉当前窗口
        gtk_container_add(GTK_CONTAINER(GTK_BOX(GTK_DIALOG(message) -> action_area)), button); //定义button这个构件在格子中的位置
        gtk_widget_show(button);

        button = gtk_button_new_from_stock(GTK_STOCK_CANCEL);                              //特定的取消键
        g_signal_connect_swapped(button, "clicked",G_CALLBACK(gtk_widget_destroy),message);
        gtk_container_add(GTK_CONTAINER(GTK_BOX(GTK_DIALOG(message) -> action_area)), button);
        gtk_widget_show(button);
        gtk_widget_show(message);
...
void login_stu_change_view(){         //学生修改个人的信息
	GtkWidget *button;
	GtkWidget *table;
	GtkWidget *label;
...
void login_admin_add_message_view(GtkWidget *widget, gpointer data){  //对不同的添加信息进行表达的函数
	...
	strcpy(id, gtk_entry_get_text(GTK_ENTRY(add.id)));                //对id进行赋值
	message = gtk_message_dialog_new(NULL, GTK_DIALOG_MODAL, GTK_MESSAGE_INFO, GTK_BUTTONS_NONE,NULL);
	button = gtk_button_new_from_stock(GTK_STOCK_OK);
	g_signal_connect_swapped(button, "clicked", G_CALLBACK(gtk_widget_destroy),message);
...
void login_admin_del_view(){    //显示删除界面， 又是毫无难点的程序。。自己看
    ...
    gtk_table_attach_defaults(GTK_TABLE(table), del.entry, 0, 1, 1, 2);
...
login_change_message_view(GtkWidget *widget, gpointer data){  //对于修改结果的不同情况的返回，也是以消息框配一个系统自带的按钮实现的
    GtkWidget *message;
    GtkWidget *button;
    char id[30];
    stu *temp = NULL;
    ...
    if(temp == NULL){
        ...
        if(find_id_exist(head, id) != NULL){
            ...
            strcpy(p->  score.C_program, gtk_entry_get_text(GTK_ENTRY(change.score_C_program)));
            strcpy(p->  score.computer, gtk_entry_get_text(GTK_ENTRY(change.score_computer)));
            strcpy(p->  score.PE, gtk_entry_get_text(GTK_ENTRY(change.score_PE)));
            ...
            strcpy(temp -> score.C_program, p -> score.C_program);
            strcpy(temp -> score.computer, p -> score.computer);
            strcpy(temp -> score.PE, p -> score.PE);
...
void login_admin_view(){      //管理员菜单，完全无难点
	...
	button = gtk_button_new_with_label("添加学生信息");
	g_signal_connect(button,"clicked",G_CALLBACK(login_add_ctrl), NULL);
	gtk_table_attach_defaults(GTK_TABLE(table), button,0, 1, 0, 1);
	gtk_widget_show(button);

	button = gtk_button_new_with_label("删除学生信息");
...
int main(int argc, char *argv[]){
	...
	strcpy(manager -> id, "烫烫烫");   //设置一个管理员23333
	strcpy(manager -> password, "tangtangtang");
